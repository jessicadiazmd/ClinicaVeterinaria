...
//Ruta /citas/crear para rpedir una cita
      ...
      if (err) {
        ...
        if (data.length === 0) {
          res.send({
            error: true,
            data: data,
            mensaje: "No hay ningún cliente con este email",
          });
        }
        //Si el email no esta registrado
        else {
          req.app.locals.db.collection("citas").insertOne(
            {
...
router.put("/modificar", (req, res) => {
  req.app.locals.db.collection("citas").updateOne(
    { email: req.body.email },
    {
      $set: {
